...
int snull_open(struct net_device *dev)
{
    ...
    /* ensure we use the device's addr_len when composing a new address */
    if (dev == snull_devs[1]) {
        unsigned char addr[IFHWADDRLEN];       /* full temp buffer */
        memset(addr, 0, IFHWADDRLEN);         /* clear any extra bytes */
        memcpy(addr, dev->dev_addr, dev->addr_len);
        addr[dev->addr_len - 1]++;            /* increment last octet */
        eth_hw_addr_set(dev, addr);           /* kernel will copy dev->addr_len bytes */
    } //------------------------------------------------------------------------------------------
	if (use_napi) {
		struct snull_priv *priv = netdev_priv(dev);
